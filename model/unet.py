...
from .ops import conv2d,conv, deconv2d, lrelu, relu,global_avg_pooling, fc, batch_norm, init_embedding, conditional_instance_norm
...
class UNet(object):
    def __init__(self, experiment_dir=None, experiment_id=0, batch_size=16, input_width=256, output_width=256,
                 ...
                 Lcategory_penalty=1.0, embedding_num=12, embedding_dim=128, input_filters=3, output_filters=3,
                 ch=64, n_layer=4, n_z=8, attr_dim=10):
        ...
        self.n_z = n_z
        self.attr_dim = attr_dim  # 添加属性向量维度
    ...
    def encoder(self, images, is_training, reuse=False):
        with tf.compat.v1.variable_scope("generator"):
            ...
            def encode_layer(x, output_filters, layer):
                ...
                enc = batch_norm(conv, is_training, name="g_e%d_bn" % layer)
    ...
    def attribute_encoder(self, x, is_training, reuse=False):
        channel = self.ch
        with tf.compat.v1.variable_scope("generator"):
            if reuse:
                tf.compat.v1.get_variable_scope().reuse_variables()
            ...
            fc_units = [channel, channel // 2]  # Define number of units in each fully-connected layer
            for i, units in enumerate(fc_units):
                x = tf.compat.v1.layers.dense(x, units=units, activation=relu, name=f'fc_{i}')

            # Output Layer
            x = tf.compat.v1.layers.dense(x, units=self.attr_dim, activation=None, name='attribute_logit')
            x = batch_norm(x, is_training, name="batch_norm")
            ...
            def decode_layer(x, output_width, output_filters, layer, enc_layer, dropout=False, do_concat=True):
                ...
                if layer != 8:
                    ...
                    if inst_norm:
                        ...
                        dec = batch_norm(dec, is_training, name="g_d%d_bn" % layer)
                if dropout:
    ...
    def generator(self, images,images_C, embeddings, embedding_ids, inst_norm, is_training, reuse=False):
        ...
        attribute_C = self.attribute_encoder(images_C, is_training=is_training,reuse=reuse)
        embedded = tf.concat([e8, local_embeddings], 3)
        embedded = tf.concat([embedded,attribute_C], 3)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        # target images
        real_B = real_data[:, :, :, self.input_filters:self.input_filters + self.input_filters]

        # attributte images
        real_C = real_data[:, :, :, self.input_filters + self.input_filters:]

        embedding = init_embedding(self.embedding_num, self.embedding_dim)
        fake_B, encoded_real_A = self.generator(real_A,real_C, embedding, embedding_ids, is_training=is_training,
                                                inst_norm=inst_norm)
        l1_loss =  tf.reduce_mean(tf.abs(fake_B - real_B))
        #l2 loss
        #l1_loss =  tf.reduce_mean(tf.square(fake_B - real_B))
    ...
    def train(self, lr=0.0002, epoch=200, schedule=10, resume=True, flip_labels=False,
        ...
        g_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(loss_handle.l1_loss, var_list=t_vars)
        tf.compat.v1.global_variables_initializer().run()
        ...
        for ei in range(epoch):
            ...
            for bid, batch in enumerate(train_batch_iter):
                ...
                loss_history = []
                loss_history.append(batch_g_loss)
        ...
        plt.savefig(os.path.join(self.log_dir, 'loss_history.png'))
